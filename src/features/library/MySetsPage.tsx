import { CreateNewStudySetCard } from "./components/NewStudySetCard";
...
export const MySetsPage = () => {
	const { queryFn } = useAuthQuery<Studyset[]>({
		endpoint: "/me/study-sets/created",
	});

	const { isLoading, data } = useQuery({
		queryKey: ["myStudySetsCreated"],
		queryFn,
	});
	...
	return (
		<>
			...
			{!sets || (sets.length === 0 && <div>No study sets</div>)}
			{sets?.map((studySet) => (
				<StudySetCard key={studySet.id} studySet={studySet} />
				// TODO: Fix something here idk
			))}
		</>
	);
};
