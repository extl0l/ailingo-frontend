...
export const NavigationLinks = () => {
  ...
  return (<nav>
    <ul className="flex gap-3.5 items-center">
      {defaultNavigationLinks.map(link => (<li key={link.to}>
...
const StyledNavigationLink = (props: StyledNavigationLinkProps) => {
  ...
  const className = 'font-medium text-xl text-theme-brown-light opacity-50 hover:opacity-100 transition-opacity relative';
  return (
...
const ActiveLinkDecoration = () => {
  return <span
      className="w-1.5 h-1.5 block bg-theme-green-light rounded-full absolute top-0 right-0 translate-x-1/2"/>;
};